...
AKDE_HR <- function(cilla,
  ...
  message("Fitting movement model for: ", unlist(cilla@info[1]))
    ...
    if(BINOMIAL == "Canis_latrans_NC"){
    
  message("Cleaning short time intervals")
  #Function for cleaning odd timestamps from data
  removeRows <- function(data,cutoff) {
    rowNum <- which(diff(data$t)/60 >=cutoff)
  ...
  #Clean the data
  cilla <- removeRows(cilla,60)
  }
  
  
  if(BINOMIAL == "Propithecus_verreauxi"){
  ...
  #Turn error on for certain species
  #if(BINOMIAL == "Procyon_lotor_Japan" | BINOMIAL == "Didelphis_virginiana"){
  #  GUESS$error <- TRUE
  #}
  ...
  FITS2 <- tryCatch(
    {
      FITS2 <- ctmm.select(cilla.split[[2]],
                           GUESS2,
                           control=list(method = "pNewton",
                                        cores = -1),
                           method = "pHREML")
  ...
  if(class(FITS2) == "list") {T_2.FIT <- FITS2[[1]]} else {
    
    T_2.FIT <- FITS2
  }
  
  if(is.null(T_1.FIT) || is.null(T_2.FIT)){same.model <- NA} else{
  OVER <- -log(overlap(list(T_1.FIT, T_2.FIT))[1,2,])
  ...
  ##########################################################################################
  ##########################################################################################
  # Cross-validate a KDE estimated on thinned data
  ##########################################################################################
  ##########################################################################################

    
  cat("Cross-validating the KDE estimated on thinned data")
  
  #Thin the data
  message("\n", "Thinning the data")
  
  #Get 3*tau_p as the thinning factor
  tau_p <- if(nrow(summary(T_1.FIT, units = FALSE)$CI) >= 2){summary(T_1.FIT, units = FALSE)$CI[2,2]*3} else {
    
    tau_p <- 1
  }
  
  #Total duration of the sampling period (in seconds)
  tot.dur <- (tail(cilla.split[[1]]$t, 1) - head(cilla.split[[1]]$t, 1))
  
  #How many times can you thin the data
  THIN <- round(tot.dur/(tau_p), 0)
  ...
  for(i in 1:THIN){
    ...
    TIME <- head(cilla.split[[1]]$t, 1) + (tau_p*(i-1))
    
    KEEPERS[i] <- which(abs(cilla.split[[1]]$t - TIME) == min(abs(cilla.split[[1]]$t - TIME)))
    
    
  }
  
  if(nrow(summary(T_1.FIT, units = FALSE)$CI) == 1){
    
    THINNED.DATA <- cilla.split[[1]]
    
  } else {
  THINNED.DATA <- cilla.split[[1]][KEEPERS,]
  }
  
  if(nrow(THINNED.DATA) >= 3){
  #Calculate the KDE HR estimate using the thinned data
  message("Cross-validating the kde home range estimates")
  ...
  THINNED.IID.FIT <- ctmm.fit(THINNED.DATA)#,
  #control=list(method = "pNewton"),
  #method = "pHREML")
  
  
  
  thinned.KDE.HR.estimate <- akde(THINNED.DATA,
                                  THINNED.IID.FIT)

  #Calculate how many points are in the KDE HR estimate
  thinned.KDE.contour <- SpatialPolygonsDataFrame.UD(thinned.KDE.HR.estimate)
  
  
  thinned.kde.res <- unname(GISTools:::poly.counts(SpatialPoints.telemetry(cilla.split[[2]]),
                                           thinned.KDE.contour))
  
  thinned.kde.percent <- round(thinned.kde.res/nrow(cilla.split[[2]])*100, 2)
  
  } else {
    
    message("Not enough data left after thinning to estimate a HR, returning NAs")
    
    
    thinned.kde.percent <- rep(NA, 3)
  }
  
    }#Closes the if check above that asks if the fits to T_1 and T_2 exist
  ...
  res$tau_v <- if(nrow(summary(FIT, units = FALSE)$CI) == 4){summary(FIT, units = FALSE)$CI[3,2]} else {NA}
  
  res$speed <- if(nrow(summary(FIT, units = FALSE)$CI) == 4){summary(FIT, units = FALSE)$CI[4,2]} else {NA}
  
  res$speed <- if(nrow(summary(FIT, units = FALSE)$CI) == 3){summary(FIT, units = FALSE)$CI[3,2]} else {res$speed}
  ...
  res$"thinned.n" <- if(exists("KEEPERS")){length(KEEPERS)} else{NA}
  
  res$"droped" <- if(exists("KEEPERS")){(nrow(cilla.split[[1]]) - length(KEEPERS))} else{NA}
  
  res$"percent.loss" <- res$"droped"/res$"full.n"

  res$"thinned.kde.95CV.min" <- thinned.kde.percent[1]
  
  res$"thinned.kde.95CV.ml"<- thinned.kde.percent[2]
  
  res$"thinned.kde.95CV.max" <- thinned.kde.percent[3]
